...
/*   Updated: 2024/04/02 07:23:35 by mateo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_transform_main(t_pt *new_pt, t_pt *point, t_fdf *fdf)
{
	...
	{
		ft_iso(new_pt);
		new_pt->x += ((MENU + (WIDTH - MENU) / 2)) - \
			fdf->x_min - (fdf->new_width / 2);
		new_pt->y += ((HEIGHT) / 2) - fdf->y_min - (fdf->new_height / 2);
	}
	else
	{
...
t_pt	*ft_transform(t_pt *point, t_fdf *fdf)
{
	...
	ft_transform_main(new_pt, point, fdf);
